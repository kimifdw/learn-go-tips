...
func newProposer(id int, value string, net network, acceptors ...int) *proposer {
	p := &proposer{
		...
		value:   value,
		net:     net,
		accepts: make(map[int]bool, len(acceptors)),
	}
	for _, acceptor := range acceptors {
		p.accepts[acceptor] = false
	}
	return p
}
...
func (p *proposer) majority() int {
	return len(p.accepts)/2 + 1
}

func (p *proposer) majorityReached() bool {
	count := 0
	for _, ok := range p.accepts {
		if ok {
			count++
		}
	}
